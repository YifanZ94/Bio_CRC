...
class SimpleSplitter():
    """
    Split a horizontally stacked array back into multiple views based on feature counts.
    Compatible with sklearn Pipeline.
    """
    def __init__(self, n_features_per_view):
    ...
    def transform(self, X):
        """Split X into multiple views based on n_features_per_view"""
        views = []
        start_idx = 0
        for n_features in self.n_features_per_view:
            end_idx = start_idx + n_features
            views.append(X[:, start_idx:end_idx])
            start_idx = end_idx
        return views
