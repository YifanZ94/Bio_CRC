...
cat_gex = pd.read_csv("gex_obs_classes.csv", delimiter=",")
cat_gex = cat_gex.fillna('sNaN')

# cat_gex['date'] = cat_gex['Unnamed: 0'].str.split('_').str[1]
...
merged = pd.merge(features, cat_gex, left_on='BC', right_on='Unnamed: 0', how = 'inner')
...
chemo_profile = pd.read_csv('top10_chemok.csv')
...
def build_model():
    model = Sequential([
        Dense(64, activation='relu', input_shape=(num_features,), kernel_regularizer=l1_l2(l1=0.01, l2=0.01)),
        Dropout(0.3),
        Dense(32, activation='relu', kernel_regularizer=l1_l2(l1=0.01, l2=0.01)),
        Dropout(0.2),
        Dense(16, activation='relu', kernel_regularizer=l1_l2(l1=0.01, l2=0.01)),
        Dropout(0.2),
        Dense(num_classes, activation='softmax', kernel_regularizer=l1_l2(l1=0.01, l2=0.01))
        # softmax pair with sparse_categorical_crossentropy
        
        # Dense(num_classes, activation='sigmoid', kernel_regularizer=l1_l2(l1=0.01, l2=0.01))
        # sigmoid pair with binary_crossentropy
    ])
